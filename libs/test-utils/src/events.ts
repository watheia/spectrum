...
export function triggerPress(element: Document | Window | Node | Element, opts = {}) {
...
export function installMouseEvent() {
  beforeAll(() => {
    ...
    global.MouseEvent = class FakeMouseEvent extends MouseEvent {
      _init: MouseEventInit | undefined
      constructor(name: string, init: MouseEventInit | undefined) {
        super(name, init)
        this._init = init
      }
      get pageX() {
        return this._init?.clientX ?? 0
      }
      get pageY() {
        return this._init?.clientY ?? 0
      }
    }
    // @ts-ignore
