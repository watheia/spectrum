...
description: This is the last post in our three part series on building a button component. In the [first post](https://watheia.org/blog/building-a-button-part-1.html), we covered how React Spectrum and React Aria implement adaptive press events across mouse, touch, keyboard, and screen readers. In the [second post](https://watheia.org/blog/building-a-button-part-2.html), we covered hover interactions. Today, we’ll cover keyboard focus behavior.
date: 2020-09-09
...
This is the last post in our three part series on building a button component. In the [first post](https://watheia.org/blog/building-a-button-part-1.html), we covered how React Spectrum and React Aria implement adaptive press events across mouse, touch, keyboard, and screen readers. In the [second post](https://watheia.org/blog/building-a-button-part-2.html), we covered hover interactions. Today, we’ll cover keyboard focus behavior.
...
However, it gets even more tricky on iOS. While on macOS, Safari will respect our programatic focus, on iOS the browser attempts to forcibly blur the element _asynchronously_ sometime after the `onClick` event is fired. This means that even programmatically, focusing the button will not work. 🤯
...
This focus normalization behavior is implemented by the [usePress](https://watheia.org/react-aria/usePress.html) hook in React Aria, which is used by [useButton](https://watheia.org/react-aria/useButton.html), [useCheckbox](https://watheia.org/react-aria/useCheckbox.html), [useRadio](https://watheia.org/react-aria/useRadioGroup.html), and many other hooks. If you’re implementing your own components, I’d highly recommend checking them out to ensure the focus behavior is consistent across browsers.
