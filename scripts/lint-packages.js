...
for (let pkg of packages) {
  ...
  if (!pkg.includes('@react-types') && !pkg.includes('@spectrum-icons')) {
    ...
    softAssert(
      !json.dependencies || !json.dependencies["@watheia/spectrum-css-temp"],
      `${pkg} has @watheia/spectrum-css-temp in dependencies instead of devDependencies`
    );
    softAssert(
    ...
    softAssert(
      !json.dependencies || !json.dependencies["@react-spectrum/test-utils"],
      "@react-spectrum/test-utils should be a devDependency"
    );
    softAssert(
    ...
    if (
      json.name.startsWith("@react-spectrum") &&
      json.devDependencies &&
      json.devDependencies["@watheia/spectrum-css-temp"]
    ) {
  ...
  softAssert.deepEqual(
    json.repository,
    { type: "git", url: "https://github.com/watheia/spectrum" },
